...
describe('/api/tournament/create', () => {
  ...
  test('Valid tournament gets created', async () => {
    await createTournamentRoute(tournament.creatorId, tournament, repository);
    expect(await repository.get(tournament.id)).toEqual(tournament);
  });

  test('Tournament is validated and returns status 400 when invalid', async () => {
  ...
  test('Returns status 500 when a valid tournament can not be created', async () => {
    // $FlowFixMe
