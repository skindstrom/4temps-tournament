...
describe('User reducer', () => {
  ...
  test('Failed login sets userId to previous value', () => {
    ...
    expect(
      reducer({ id }, makePackAction(LIFECYCLE.FAILURE, 'LOGIN_USER'))
    ).toEqual({ id });
  });

  test('Successful logout sets id to empty string', () => {
    expect(
      reducer({ id }, makePackAction(LIFECYCLE.SUCCESS, 'LOGOUT_USER'))
    ).toEqual({ id: '' });
  });

  test('Failed logout makes does not impact impact', () => {
    expect(
      reducer({ id }, makePackAction(LIFECYCLE.FAILURE, 'LOGOUT_USER'))
    ).toEqual({ id });
