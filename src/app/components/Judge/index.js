...
function mapStateToProps({tournaments, rounds}: ReduxState) {
  return {
    ...
    hasActiveDance: (tournaments.forJudge !== '') &&
      hasActiveDance(
        tournaments.byId[tournaments.forJudge].rounds
...
function mapDispatchToProps(dispatch: ReduxDispatch) {
  return {
    load: () => dispatch(
      {type: 'GET_JUDGE_TOURNAMENT', promise: getTournamentForJudge()}
    )
  };
}
...
function isRoundActive(round: Round): boolean {
  return round.active &&
    round.groups.reduce((a,b) => a || isGroupActive(b), false);
}
